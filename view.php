...
<body>
				...
				<div class="wthree_tab_content_pos">
					<div class="wthree_tab_content_pos_grid">
						...
						<ul class="address">
							...
							<li>
								<ul class="address-text">
									<li><b>Url Access : </b></li>
									<li><?= $_SESSION['userAccessRule']['url'] ?></li>
								</ul>
							</li>
							<li>
								<ul class="address-text">
	...
	<section class="content-url">
		<?php
			$urlRequest = $_GET['url'];
			if ($_SESSION['userAccessRule']['url'] && $_SESSION['userAccessRule']['url'] === $urlRequest){
		?>
		<iframe id="frame" src="http://localhost:8888/google-login/proxy.php?url=<?= $urlRequest ?>"></iframe>
		<?php } else { ?>
			<h2>No url or not permision </h2>
		<?php } ?>
	</section>
